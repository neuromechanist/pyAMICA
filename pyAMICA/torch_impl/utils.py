...
def load_eeglab_data(
    ...
    with open(filepath, 'rb') as f:
        ...
        # Reshape to (field_dim, data_dim) and transpose
        # EEGLAB stores in column-major (Fortran) order
        data = data.reshape((field_dim, data_dim), order='F').T
        
    return data


def save_pytorch_results(
    model,
    output_dir: str,
    ...
    if save_history and hasattr(model, 'll_history'):
        ...
        with open(output_dir / 'summary.txt', 'w') as f:
            f.write(f"PyTorch AMICA Results\n")
            f.write(f"=" * 50 + "\n")
