...
class TestAMICATorch(unittest.TestCase):
    ...
    def test_preprocessing(self):
        ...
        model = AMICATorch(n_channels=self.n_channels, device=self.device)

        # Test mean removal
        X_centered = model.preprocess_data(self.data, do_mean=True, do_sphere=False)

        mean_after = X_centered.mean(dim=1)
        self.assertTrue(
            torch.allclose(mean_after, torch.zeros_like(mean_after), atol=1e-6)
        )

        # Test sphering
        X_sphered = model.preprocess_data(self.data, do_mean=True, do_sphere=True)

        cov = torch.cov(X_sphered)
        # Check if covariance is close to identity
        self.assertTrue(
            torch.allclose(
                cov, torch.eye(self.n_channels, device=self.device), atol=0.1
            )
        )
...
class TestGaussianMixtureICA(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertTrue(
            torch.allclose(
                gmm.alpha.sum(dim=0), torch.ones(self.n_sources, device=self.device)
            )
        )
    ...
    def test_forward(self):
        ...
        gmm = GaussianMixtureICA(
            n_sources=self.n_sources, n_mix=self.n_mix, device=self.device
        )
...
class TestOptimizers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = AMICATorch(
            n_channels=self.n_channels, n_models=1, n_mix=2, device=self.device
        )
    ...
    def test_natural_gradient(self):
        ...
        optimizer = NaturalGradientOptimizer(
            self.model.parameters(), lr=0.01, fisher_type="diagonal"
        )
    ...
    def test_newton_optimizer(self):
        ...
        newton_opt = NewtonOptimizer(self.model, method="cg", max_iter=5)

        # Get initial loss (smoke test: must not raise)
        with torch.no_grad():
            self.model.compute_log_likelihood(self.X)

        # Newton step
        newton_opt.step(self.X)

        # Get final loss (smoke test: must not raise)
        with torch.no_grad():
            self.model.compute_log_likelihood(self.X)
...
class TestUtils(unittest.TestCase):
    ...
    def test_find_permutation(self):
        ...
        corr = np.array([[0.1, 0.9, 0.2], [0.8, 0.2, 0.1], [0.2, 0.1, 0.95]])
