"""Unit tests for ``pamica.metrics.pmi`` (issue #135, PMI metric phase 2).

PMI is backend-agnostic pure NumPy, so it lives directly under
``pamica/tests/`` (like ``test_metrics_mir.py``). Per the NO-MOCK policy,
...
from pamica.metrics._common import resolve_nbins
from pamica.metrics.pmi import (
