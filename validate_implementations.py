...
def run_fortran_amica(data: np.ndarray, params: Dict, output_dir: Path, seed: int) -> Dict:
    ...
    if not binary_path.exists():
        print(f"Warning: Fortran binary not found at {binary_path}. Skipping Fortran comparison.")
        return None
    ...
    if W_file.exists():
        ...
        try:
            ...
            if len(W) == n_sources * n_sources:
                results['W'] = W.reshape(n_sources, n_sources, order='F')  # Fortran order
            elif len(W) == n_sources * n_sources * params.get('num_models', 1):
                # Multiple models, take first one
                W_all = W.reshape(n_sources, n_sources, params.get('num_models', 1), order='F')
                results['W'] = W_all[:, :, 0]
        except Exception as e:
    ...
    if A_file.exists():
        try:
            ...
            if len(A) == n_sources * n_sources:
                results['A'] = A.reshape(n_sources, n_sources, order='F')  # Fortran order
            elif len(A) == n_sources * n_sources * params.get('num_models', 1):
                # Multiple models, take first one
                A_all = A.reshape(n_sources, n_sources, params.get('num_models', 1), order='F')
                results['A'] = A_all[:, :, 0]
        except Exception as e:
...
def run_pytorch_amica(data: np.ndarray, params: Dict, output_dir: Path, seed: int) -> Dict:
    """Run PyTorch AMICA and collect results."""
    print("Running PyTorch AMICA...")
    ...
    # Create model with same initialization
    model = AMICATorch(
    ...
    torch.manual_seed(seed)
    with torch.no_grad():
        # Set initial mixing matrix to identity (like Fortran)
        for h in range(model.n_models):
            model.A[h].data = torch.eye(model.n_channels, model.n_sources, device=device)
            model.c[h].data.zero_()
        
        # Initialize mixture parameters consistently
        model.log_alpha.data.zero_()
        model.mu.data.zero_()
        model.log_beta.data.zero_()
        model.rho_logit.data.zero_()
    
    # Run optimization
    pytorch_output = output_dir / 'pytorch_output'
    ...
    model.fit(
        ...
        debug=True,  # Use debug mode for detailed output
        output_dir=str(pytorch_output),
...
def compare_results(fortran_results: Optional[Dict], pytorch_results: Dict) -> Dict:
    ...
    if 'W' in fortran_results and 'W' in pytorch_results:
        ...
        if W_fortran.shape == W_pytorch.shape:
            ...
            W_fortran_norm = W_fortran / (np.linalg.norm(W_fortran, axis=1, keepdims=True) + 1e-10)
            W_pytorch_norm = W_pytorch / (np.linalg.norm(W_pytorch, axis=1, keepdims=True) + 1e-10)
            ...
            if 'A' in fortran_results and 'A' in pytorch_results:
                ...
                if A_fortran.shape == A_pytorch.shape:
                    ...
                    comparison['mixing_matrix_error'] = A_diff / A_norm if A_norm > 0 else float('inf')
    
    return comparison


def print_comparison_report(comparison: Dict, fortran_results: Optional[Dict], pytorch_results: Dict):
...
def main():
    ...
    parser.add_argument('--skip-fortran', action='store_true', help='Skip Fortran comparison')
